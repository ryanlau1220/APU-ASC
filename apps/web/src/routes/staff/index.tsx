...
export const Route = createFileRoute('/staff/')({
  component: StaffDashboardContent,
})
  ...
  const metrics = [
    {
      ...
      subtitle: `${appointments.filter((a) => a.status === 'PENDING').length} pending check-in`,
      icon: Calendar,
      color: 'text-primary bg-primary/10 border-primary/30',
    },
    {
      title: 'Active Invoices',
      value: String(payments.filter((p) => p.paymentStatus !== 'PAID').length),
      subtitle: 'Unpaid billing',
      icon: CreditCard,
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground transition-colors">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <section className="bg-card border border-border rounded-xl p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/30 text-xs font-semibold text-primary">
              ...
              Front Counter Operations Console
            </div>
            <h1 className="font-heading text-2xl sm:text-3xl font-bold tracking-tight">
              Staff Service Desk & Customer Intake
            </h1>
            <p className="text-sm text-muted-foreground">
              Manage walk-in appointment check-ins, register customer accounts,
              generate billing invoices, and process counter payments.
            </p>
          </div>
        ...
        {/* Counter Check-in Table */}
        <section className="bg-card border border-border rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4 text-primary" />
              <h2 className="font-heading text-lg font-bold">
                Counter Arrivals Queue
              </h2>
            </div>
            <Link
              to="/staff/appointments"
          ...
          {appointments.length === 0 ? (
            <div className="text-center py-8 text-xs text-muted-foreground">
              No appointments registered in queue.
            </div>
          ) : (
