...
public class AppointmentService {
    ...
    public Appointment createAppointment(String customerId, String counterStaffId,
                                         String serviceId, String vehiclePlate,
        ...
        Appointment appt = new Appointment(id, customerId, counterStaffId,
                "UNASSIGNED", serviceId, vehiclePlate, dateTime, ApptStatus.PENDING, "");
        appointmentDAO.save(appt);
        SystemLogger.log(actorId(), "CREATE_APPT", id,
    ...
    public boolean assignTechnician(String appointmentId, String technicianId) {
        ...
        if (technicianHasConflict(technicianId, appointmentId,
                appt.getAppointmentDateTime(), service.getType())) return false;

        appt.markAsAssigned(technicianId);
        appointmentDAO.save(appt);
        SystemLogger.log(actorId(), "ASSIGN_TECHNICIAN", appointmentId,
    ...
    public boolean completeAppointment(String appointmentId, String technicianNotes) {
        ...
        appointmentDAO.save(appt);
        SystemLogger.log(actorId(), "COMPLETE_APPT", appointmentId,
    ...
    private boolean hasConflict(String excludeApptId, LocalDateTime dateTime, ServiceType type) {
        ...
        return appointmentDAO.getAll().stream()
                ...
                .anyMatch(a -> {
                    ...
                    int existDur = (s != null && s.getType() == ServiceType.MAJOR)
                            ? MAJOR_DURATION_HOURS : NORMAL_DURATION_HOURS;
                    LocalDateTime existEnd = a.getAppointmentDateTime().plusHours(existDur);
                    return dateTime.isBefore(existEnd)
                            && newEnd.isAfter(a.getAppointmentDateTime());
                });
    }

    private boolean technicianHasConflict(String technicianId, String excludeApptId,
                                           LocalDateTime dateTime, ServiceType type) {
        int duration = type == ServiceType.MAJOR ? MAJOR_DURATION_HOURS : NORMAL_DURATION_HOURS;
        LocalDateTime newEnd = dateTime.plusHours(duration);
