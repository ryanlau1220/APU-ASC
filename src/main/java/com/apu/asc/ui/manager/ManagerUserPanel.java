...
public class ManagerUserPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        roleFilter = new JComboBox<>(new String[]{"ALL", "MANAGER", "STAFF", "TECHNICIAN", "CUSTOMER"});
    ...
    private void openAddDialog() {
        ...
        p.add(new JLabel("Role:"));     p.add(roleCombo);
        ...
        int res = JOptionPane.showConfirmDialog(this, p, "Add User",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        if (res != JOptionPane.OK_OPTION) return;

        Role role = Role.valueOf((String) roleCombo.getSelectedItem());
        User created = userService.createUser(role, username.getText(),
                new String(pass.getPassword()), fullName.getText(),
                email.getText(), contact.getText());

        if (created == null) {
            JOptionPane.showMessageDialog(this, "Username already taken.", "Error",
    ...
    private void openEditDialog() {
        ...
        int res = JOptionPane.showConfirmDialog(this, p, "Edit User",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        if (res != JOptionPane.OK_OPTION) return;
    ...
    private void handleDeactivate() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this,
                "Deactivate user \"" + name + "\"?", "Confirm", JOptionPane.YES_NO_OPTION);
    ...
    private void handleReactivate() {
        ...
        String name = (String) model.getValueAt(row, 2);
        int confirm = JOptionPane.showConfirmDialog(this,
                "Reactivate user \"" + name + "\"?", "Confirm", JOptionPane.YES_NO_OPTION);
        if (confirm != JOptionPane.YES_OPTION) return;
