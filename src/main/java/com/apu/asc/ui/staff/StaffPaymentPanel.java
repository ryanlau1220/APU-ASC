...
public class StaffPaymentPanel extends JPanel {
    ...
    private void handleProcess() {
        ...
        if (p == null) {
            JOptionPane.showMessageDialog(this,
                    "Payment failed. Appointment may not be COMPLETED.", "Error",
                    JOptionPane.ERROR_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this,
                    "Payment recorded: " + p.getPaymentId()
                            + "\nAmount: RM " + String.format("%.2f", p.getAmountPaid()));
            loadData();
        }
    }
    ...
    private void handleSendReceipt() {
        ...
        if (!ok) {
            JOptionPane.showMessageDialog(this,
                    "Failed to send receipt. Check config.properties for SMTP settings.", "Error",
                    JOptionPane.ERROR_MESSAGE);
        } else {
